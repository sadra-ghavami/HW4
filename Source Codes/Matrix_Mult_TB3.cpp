...
void Matrix_Mult_TB3::clk_generation() {
	while (true) {
		clk = SC_LOGIC_0;
		wait(10, SC_NS);
		clk = SC_LOGIC_1;
...
void Matrix_Mult_TB3::rst_generation() {
	rst = SC_LOGIC_0;
	wait(2, SC_NS);
	rst = SC_LOGIC_1;
	wait(5, SC_NS);
...
void Matrix_Mult_TB3::input_generation() {
...
void Matrix_Mult_TB3::print_output_when_ready() {
	if (done == SC_LOGIC_1) {
		cout << "result RTL test 3: " << endl;
		for (int i = 0; i < 4; i++) {
			cout << final_result[0][i].read().to_uint();
			for (int j = 1; j < 2; j++) {
				cout << " , " << final_result[j][i].read().to_uint();
			}
			cout << endl;
		}
	}
